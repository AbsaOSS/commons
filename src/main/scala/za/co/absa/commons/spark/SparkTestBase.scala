...
import za.co.absa.commons.time.TimeZoneNormalizer
...
trait SparkTestBase { self =>
  ...
  val sparkMaster: String = config.getString("sparkTestBase.testBaseMaster")
  ...
  implicit val spark: SparkSession = if (sparkMaster == "yarn") {
    val confDir = config.getString("sparkTestBase.hadoop.conf.dir")
    val distJarsDir = config.getString("sparkTestBase.spark.distJars.dir")
    val sparkHomeDir = config.getString("sparkTestBase.spark.home.dir")
