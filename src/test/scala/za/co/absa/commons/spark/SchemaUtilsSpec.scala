...
class SchemaUtilsSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll with SparkTestBase {
  ...
  val jsonDFA = """[{"id":1,"legs":[{"legid":100,"conditions":[{"checks":[{"checkNums":["1","2","3b","4","5c","6"]}],"amount":100}]}], "key" : {"alfa": "1", "beta": {"beta2": "2"}} }]"""
  val jsonDFB = """[{"id":1,"legs":[{"legid":100,"conditions":[{"checks":[{"checkNums":["1","2","3b","4","5c","6"]}],"amount":100,"price":10}]}]}]"""
  val jsonDFC = """[{"legs":[{"legid":100,"conditions":[{"amount":100,"checks":[{"checkNums":["1","2","3b","4","5c","6"]}]}]}],"id":1, "key" : {"beta": {"beta2": "2"}, "alfa": "1"} }]"""
  val jsonDFD = """[{"legs":[{"legid":100,"conditions":[{"amount":100,"checks":[{"checkNums":["1","2","3b","4","5c","6"]}]}]}],"id":1, "key" : {"beta": {"beta2": 2}, "alfa": 1} }]"""

  behavior of "isSameSchema"

  it should "say true for the same schemas" in {
    val dfA1 = spark.read.json(Seq(jsonDFA).toDS)
    val dfA2 = spark.read.json(Seq(jsonDFA).toDS)
  ...
  it should "say false when first schema has an extra field" in {
    val dfA = spark.read.json(Seq(jsonDFA).toDS)
    val dfB = spark.read.json(Seq(jsonDFB).toDS)
  ...
  it should "order schemas for equal schemas" in {
    val dfA = spark.read.json(Seq(jsonDFA).toDS)
    val dfC = spark.read.json(Seq(jsonDFC).toDS).select("legs", "id", "key")
  ...
  it should "produce a list of differences with path for schemas with different columns" in {
    val schemaA = spark.read.json(Seq(jsonDFA).toDS).schema
    val schemaB = spark.read.json(Seq(jsonDFB).toDS).schema
  ...
  it should "produce a list of differences with path for schemas with different column types" in {
    val schemaC = spark.read.json(Seq(jsonDFC).toDS).schema
    val schemaD = spark.read.json(Seq(jsonDFD).toDS).schema
  ...
  it should "produce an empty list for identical schemas" in {
    val schemaA = spark.read.json(Seq(jsonDFA).toDS).schema
    val schemaB = spark.read.json(Seq(jsonDFA).toDS).schema
